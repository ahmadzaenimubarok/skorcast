{{--
BRACKET WASIT: tampilan bracket untuk wasit.
- TIDAK ada tombol manajemen (edit/delete/generate tim).
- Match ONGOING -> tombol "Input Skor" ke scoreboard.show (mode KONTROL, wasit bisa ubah skor).
- Match PENDING -> badge "Belum mulai" (tanpa tombol).
- Match COMPLETED -> nama pemenang (readonly, sesuai publik).
--}}
                            ...
                            <div @if($match->status === 'completed' && $match->winner) wire:click="openMatchDetail({{ $match->id }})" @endif
                                ...
                                @if($match->status === 'ongoing')
                                    <a href="{{ route('scoreboard.show', $match->id) }}"
                                       class="mt-auto block w-full h-9 flex items-center justify-center gap-1.5 text-xs font-semibold bg-emerald-600 hover:bg-emerald-500 text-white border-t border-emerald-700/40 transition-colors no-underline">
                                        <span class="w-1.5 h-1.5 rounded-full bg-white polling-indicator"></span>
